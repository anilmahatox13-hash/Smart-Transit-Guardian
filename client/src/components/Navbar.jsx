...
import { Bus, Navigation, Radio, LayoutGrid, LogOut, Sun, Moon, Globe, ChevronDown, MapPin, Settings as SettingsIcon } from 'lucide-react';

const Navbar = () => {
  const { user, logout } = useAuth();
  const { theme, toggleTheme } = useTheme();
  const { lang, setLang, t } = useLanguage();
  const { country, province, flag } = useRegion();
  ...
  return (
    <>
      <header className="sticky top-0 z-40 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 transition-colors shadow-2xs">
          ...
          <div className="flex items-center justify-between h-16">
              ...
              <nav className="hidden md:flex items-center space-x-1">
                <Link
                  ...
                  className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition ${
                    isActive('/') ? 'bg-slate-100 dark:bg-slate-800 text-emerald-600 dark:text-emerald-400' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800'
                  }`}
                >
                ...
                {(user.role === 'driver' || user.role === 'admin') && (
                  <Link
                    to="/driver"
                    className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition ${
                      isActive('/driver') ? 'bg-slate-100 dark:bg-slate-800 text-amber-600 dark:text-amber-400' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800'
                    }`}
                  >
                ...
                {user.role === 'admin' && (
                  <Link
                    to="/admin"
                    className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition ${
                      isActive('/admin') ? 'bg-slate-100 dark:bg-slate-800 text-purple-600 dark:text-purple-400' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800'
                    }`}
                  >
            ...
            <div className="flex items-center space-x-2 sm:space-x-2.5">
              ...
              <button
                ...
                className="flex items-center space-x-1.5 px-2.5 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 text-xs font-semibold border border-slate-200 dark:border-slate-700 transition"
                title="Change Region"
              >
                <span>{flag}</span>
              ...
              {user ? (
                <div className="relative pl-1 border-l border-slate-200 dark:border-slate-800">
                  <button
                    ...
                    className="flex items-center space-x-1.5 p-1 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800"
                  >
                ...
                <Link
                  to="/login"
                  className="text-xs font-semibold bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-1.5 rounded-xl transition shadow-xs"
                >
                  {t('signIn')}
                </Link>
              )}
